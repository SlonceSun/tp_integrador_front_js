...
var dtoStudent = 0.8; /* Esto es un 80% */
var dtoTrainee = 0.5; /* Esto es un 50% */
...
var formu = document.getElementById('form_tkts_comp');
formu.addEventListener('submit', function (evt) {
	evt.preventDefault();
	evt.stopPropagation();
});
...
function montoTotalPagar() {
	...
	if (categoria.value == 1) {
		totalValorTkt =
			totalValorTkt *
			(1 - dtoStudent).toFixed(
				2
			); /* NOTA: Coloque toFixed porque sino por algun motivo el valor total daba 39.999999 */
	}
